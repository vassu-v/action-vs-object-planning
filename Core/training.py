...
class PolicyNet(nn.Module):
    def __init__(self, input_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(256, len(ACTION_TYPES))
        )
...
class FileArgNet(nn.Module):
    def __init__(self, input_dim, max_files):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, 256),
            nn.ReLU(),
...
class DirArgNet(nn.Module):
    def __init__(self, input_dim, max_dirs):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, 256),
            nn.ReLU(),
...
def train_models(dataset, epochs=20, verbose=True):
    ...
    policy_input_dim = Xp.shape[1]
    policy = PolicyNet(policy_input_dim).to(DEVICE)
    ...
    file_input_dim = file_X.shape[1]
    dir_input_dim = dir_X.shape[1]
    MAX_FILES = int(file_Y.max()) + 1 if len(file_Y) > 0 else 1
    MAX_DIRS = int(dir_Y.max()) + 1 if len(dir_Y) > 0 else 1
    
    file_net = FileArgNet(file_input_dim, MAX_FILES).to(DEVICE)
    dir_net = DirArgNet(dir_input_dim, MAX_DIRS).to(DEVICE)
