...
def run_task_with_trace(task, models, planner_fn, binder_fn=None, max_steps=5, verbose=True, curiosity=None):
    """
    ...
    Args:
        ...
        curiosity: Optional CuriosityModule instance for intrinsic reward.
    """
    ...
    for step in range(max_steps):
        ...
        # 3. Curiosity Blending
        cur_bonus = 0.0
        if curiosity is not None:
            score, cur_bonus = curiosity.blend_score(score, snapshot)
        ...
        # Update curiosity buffer AFTER action
        if curiosity is not None:
            curiosity.update(snapshot)
        ...
        if verbose:
            ...
            print(f"[{step}] ACTION = {act_str:6s} {str(args):20s} | {status} | score = {score:.3f} | nov = {cur_bonus:.3f}")
