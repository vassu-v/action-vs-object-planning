import random
import string
from .env import ROOT, create_world, Action, apply_action
from .perception import encode_world

def random_text(n=20):
    return ''.join(random.choice(string.ascii_lowercase + " ") for _ in range(n))

# Canonical filenames for stability and diversity
CANONICAL_FILES = [
    "docs/notes.txt",
    "docs/todo.txt",
    "docs/report_A.txt",
    "docs/report_B.txt",
    "docs/summary_day1.txt",
    "docs/summary_day2.txt",
    "logs/sum.log",
    "logs/system.log",
    "data/dataset_01.csv",
    "data/dataset_02.csv",
    "tmp/scratch_1.txt",
    "tmp/scratch_2.txt"
]

def random_text(n=20):
    return ''.join(random.choice(string.ascii_lowercase + " ") for _ in range(n))

def traj_append_random():
    file_path = random.choice([
        "docs/notes.txt",
        "docs/todo.txt"
    ])
    return [
        Action("WRITE", (file_path, "\n" + random_text()))
    ]


def traj_run_and_store():
    # Use stable report names to ensure stable indexing
    report_name = random.choice(["report_A.txt", "report_B.txt"])
    return [
        Action("RUN", ("scripts/sum.py",)),
        Action("CREATE", ("docs", report_name, "Result:\n")),
        Action("WRITE", (f"docs/{report_name}", "See logs/sum.log\n"))
    ]

# NOTE: These trajectories are heuristic and formulaic.
# They do NOT represent optimal planning or intelligent reasoning.
# Training data consists of valid but non-optimal trajectories, 
# sufficient to seed predictive models but not encode planning competence.

def traj_make_summary():
    notes = "meeting at 3pm\n"
    todo = "- buy milk\n- finish project\n"

    # Use stable summary names
    summary_name = random.choice(["summary_day1.txt", "summary_day2.txt"])

    return [
        Action("CREATE", ("docs", summary_name, "SUMMARY\n")),
        Action("WRITE", (f"docs/{summary_name}", notes)),
        Action("WRITE", (f"docs/{summary_name}", todo)),
    ]

def traj_fail_write():
    # Failing trajectory: Writing to non-existent dir
    return [
        Action("WRITE", ("nonexistent/test.txt", "content"))
    ]

def traj_move_random():
    # Diversity: Move files between folders
    target = random.choice(["docs/notes.txt", "docs/todo.txt"])
    dest = random.choice(["tmp", "data"])
    return [
        Action("MOVE", (target, dest))
    ]

def traj_write_data():
    # Diversity: Write to data folder
    target = random.choice(["data/dataset_01.csv", "data/dataset_02.csv"])
    return [
        Action("CREATE", ("data", target.split("/")[-1], "id,val\n1,0.5\n")),
        Action("WRITE", (target, "2,0.8\n"))
    ]

def traj_create_tmp():
    # Diversity: Create temp scratch files
    target = random.choice(["tmp/scratch_1.txt", "tmp/scratch_2.txt"])
    return [
        Action("CREATE", ("tmp", target.split("/")[-1], "scratchpad\n")),
        Action("REPLACE", (target, "wiped\n"))
    ]
    
def traj_move_assets_random():
    # Neutral Noise: Move asset files to tmp/data
    # This teaches MOVE verb without conflicting with task objects (src/bin/logs)
    asset_file = random.choice(["assets/logo.png", "assets/icon.ico", "web/style.css"])
    dest_dir = random.choice(["tmp", "data"])
    return [
        Action("MOVE", (asset_file, dest_dir))
    ]

# Generic category moves removed to eliminate data ambiguity.
# Target: Isolate Object Binding Entropy from Data Ambiguity.





def traj_refactor_code():
    # Move source to tests (Maintenance)
    return [
        Action("READ", ("src/main.py",)),
        Action("MOVE", ("src/main.py", "tests"))
    ]

def traj_archive_logs():
    # Archive logs (Cleanup)
    log_file = random.choice(["logs/error.log", "logs/server.log"])
    return [
        Action("READ", (log_file,)),
        Action("MOVE", (log_file, "tmp"))
    ]

def traj_deploy_app():
    # Deploy bin to web (Deployment)
    return [
        Action("REPLACE", ("config/settings.yaml", "env: prod\n")),
        Action("MOVE", ("bin/app.exe", "web"))
    ]


def traj_backup_config():
    # Backup settings (Configuration)
    return [
        Action("READ", ("config/settings.yaml",)),
        Action("MOVE", ("config/settings.yaml", "tmp"))
    ]

def traj_run_tests():
    # Run tests (Verification)
    return [
        Action("READ", ("tests/test_main.py",)),
        Action("RUN", ("tests/test_main.py",))
    ]

def snapshot_world():
    """
    Capture the creature's full perception of the world.
    """
    objects, global_vec = encode_world(ROOT)
    return {
        "objects": objects,
        "global": global_vec
    }


def rollout_trajectory(action_sequence):
    data = []

    for action in action_sequence:
        before = snapshot_world()
        apply_action(action)
        after = snapshot_world()

        data.append({
            "state_before": before,
            "action": action,
            "state_after": after
        })

    return data


import contextlib
import io

def generate_dataset(n_standard=50, n_dense=50):
    """
    Generates a synthetic dataset of agent trajectories.
    Allows specifying the number of trajectories for each environment type.
    """
    dataset = []

    standard_generators = [
        traj_append_random,
        traj_run_and_store,
        traj_run_and_store, # Upweight success cases
        traj_make_summary,
        traj_fail_write,
        traj_move_random,
        traj_write_data,
        traj_create_tmp,
    ]

    dense_generators = [
        # Action diversity baseline (Neutral Noise)
        traj_move_random,
        traj_move_assets_random,
        traj_write_data,
        traj_create_tmp,
        # Task-specific patterns (Pure)
        traj_refactor_code,
        traj_archive_logs,
        traj_deploy_app,
        traj_backup_config,
        traj_run_tests
    ]

    # Phase 1: Standard Environment
    if n_standard > 0:
        print(f"Generating {n_standard} trajectories in STANDARD environment...")
        for _ in range(n_standard):
            # Silence the internal create_world print
            with contextlib.redirect_stdout(io.StringIO()):
                create_world(config="standard")
            
            traj_fn = random.choice(standard_generators)
            actions = traj_fn()
            dataset.extend(rollout_trajectory(actions))

    # Phase 2: Dense Environment
    if n_dense > 0:
        print(f"Generating {n_dense} trajectories in DENSE environment...")
        for _ in range(n_dense):
            # Silence the internal create_world print
            with contextlib.redirect_stdout(io.StringIO()):
                create_world(config="dense")
                
            traj_fn = random.choice(dense_generators)
            actions = traj_fn()
            dataset.extend(rollout_trajectory(actions))

    return dataset

if __name__ == "__main__":
    dataset = generate_dataset(200)
    len(dataset)

