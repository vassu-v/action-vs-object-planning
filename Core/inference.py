...
def score_transition(snapshot, imagined_global, committed_object, action_type, goal, effect_fn):
    ...
    if committed_object is not None:
        ...
        if target_key in effects:
             # CRITICAL: Only reward FILE effects, not directories
             if target_key.startswith("file:"):
                 if "logs" in target_key or "tmp" in target_key:
                     score += 25.0 * goal.get("logs", 0.0)
                     score += 15.0 * goal.get("maint", 0.0) 
                     score += 15.0 * goal.get("archive", 0.0)
    ...
    # 3. Variant A Goal Prior (Restored)
    # Applied ONLY to Variant A to compensate for lack of object commitment.
    # Variant B must learn from object-specific signals.
    if committed_object is None:
        if action_type == "RUN":
            score += 10.0 * goal.get("logs", 0.0)
            score += 15.0 * goal.get("maint", 0.0) 
            score += 15.0 * goal.get("test", 0.0)  
        if action_type == "MOVE":
            score += 15.0 * goal.get("maint", 0.0) 
            score += 15.0 * goal.get("deploy", 0.0) 
            score += 15.0 * goal.get("config", 0.0)
