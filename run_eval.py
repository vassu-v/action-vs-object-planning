...
def main():
    ...
    parser.add_argument("--online", action="store_true", help="Enable Online Learning (Selective Plasticity)")
    ...
    replay_buffer = None
    if args.online:
        print(">> ONLINE LEARNING ENABLED: Selective Plasticity Active")
        print(">> Strategy: Trajectory-Based Filtering (Student-Driver)")
        learner = OnlineLearner(models, learning_rate=1e-4) # Higher LR for fast adaptation in few shots
    ...
    for i in range(args.runs):
        ...
        # --- Online Learning Step (Refined Triggers) ---
        if args.online and replay_buffer is not None:
            if is_success:
                print(f">> SUCCESS: Adding full trajectory ({len(result['trace'])} steps)")
                replay_buffer.add_trajectory(result["trace"])
            else:
                # Learn from partial progress: filter steps where score > 0.5
                # This ensures we don't discard useful sub-actions in failed runs.
                useful_steps = [t for t in result["trace"] if t["score"] > 0.5]
                if useful_steps:
                    print(f">> PROGRESS: Adding {len(useful_steps)} useful steps from failed run.")
                    replay_buffer.add_trajectory(useful_steps)
                else:
                    print(">> NOISE: Failure discarded.")
            
            # Batch Accumulation: Train once we have enough diverse data
            MIN_BATCH = 16
            if len(replay_buffer) >= MIN_BATCH:
                ...
                print(f">> ACCUMULATING: Buffer size {len(replay_buffer)}/{MIN_BATCH}")
        # -----------------------------------------------
