...
def action_object_mask(action_type, object_index, snapshot):
    ...
    for obj, i in object_index.items():
        ...
        if action_type == "RUN":
            if is_existent and (obj.startswith("file:scripts/") or obj.startswith("file:tests/")):
                mask[i] = 1.0
            ...
            if is_existent and (obj.startswith("file:") or obj.startswith("dir:")):
                mask[i] = 1.0
...
def predict_object_effects(models, snapshot, action_type, top_k=5):
    ...
    # 🔑 Threshold-based masking (Restored)
    # Remove Top-K noise. Only predict effects the model is confident in via Training.
    return {
